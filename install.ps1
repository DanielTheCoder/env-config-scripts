# Adapted from https://github.com/dahlbyk/posh-git/blob/master/install.ps1

param([switch]$WhatIf = $false)

if($PSVersionTable.PSVersion.Major -lt 2) {
    Write-Warning "env-config requires PowerShell 2.0 or better; you have version $($Host.Version)."
    return
}
...
if(!(. (Join-Path $installDir "helper/GitCheckVersion.ps1"))) {
    return
}
...
if(Select-String -Path $PROFILE -Pattern $profileLine -Quiet -SimpleMatch) {
    Write-Host "It seems env-config is already installed..."
    return
}

Write-Host "Adding env-config to profile..."
@"

# Load env-config example profile
$profileLine

"@ | Out-File $PROFILE -Append -WhatIf:$WhatIf -Encoding (Get-FileEncoding $PROFILE)

Write-Host 'env-config sucessfully installed!'
